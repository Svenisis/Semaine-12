...
function announce_player(){
  if(count % 2 == 0){
    current_player = 2;
    $player_board.innerHTML = "<h3> Le joueur 2 est dans la place ! </h3>";
  }else{
    current_player = 1;
    $player_board.innerHTML = "<h2> Le joueur 1 est dans la place ! </h2>";
  }
}

function announce_winner(){
  if(winner == 0){
    $intel_board.innerHTML= "<h5>Vous êtes nul go refaire non mais oh  ! </h5>";
  }else if(winner == 1){
    player1_victory++;
...
function announce_partie_ended(){
  $intel_board.innerHTML = "<h4> La partie est terminer Phase Suivante !! </h4>";
  count = 1;
}
...
function play(e){
  if (game_started){
    ...
    if(count > col_array.length - 1){
      ...
      $intel_board.innerHTML = "<h4> Le 2eme tour va commencer ! </h4>"
...
function reset() {
  ...
  game_started = true;
  game_ended = false;
  console.log('Refaire une partie')
  current_player = 1;
  announce_player();
